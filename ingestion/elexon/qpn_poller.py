...
def run(conn, from_date=None, to_date=None):
    """Validate one QPN window, quarantine rejected rows and load compatible rows.
    ...
    validation_findings = validate_rows(result, spec=QPN_SPEC)
    error_rows = []
    valid_rows = []
    warning_findings = []
    warning_grouped = {}
...
def quarantine_rows(rows, conn, retrieved_at, request_context):
    """Insert rejected QPN findings into the endpoint quarantine and commit."""
...
def load(results, conn):
    ...
    insert_sql = (
        "INSERT INTO raw.elexon_qpn (settlement_date, settlement_period, time_from, time_to, level_from, "
